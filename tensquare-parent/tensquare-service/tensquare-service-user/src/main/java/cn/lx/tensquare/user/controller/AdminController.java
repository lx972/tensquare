...
public class AdminController {
    ...
    public PageResult<Admin> findPage(@RequestBody(required = false) @ApiParam(name = "Admin对象",value = "传入JSON数据",required = false) Admin admin, @PathVariable  int page, @PathVariable  int size){
        //调用AdminService实现分页条件查询Admin
        Page<Admin> pageInfo = adminService.findPage(admin, page, size);
        return new PageResult<Admin>(pageInfo.getTotal(),pageInfo.getRecords());
    }
    ...
    public PageResult<Admin> findPage(@PathVariable  int page, @PathVariable  int size){
        //调用AdminService实现分页查询Admin
        Page<Admin> pageInfo = adminService.findPage(page, size);
        return new PageResult<Admin>(pageInfo.getTotal(),pageInfo.getRecords());
    }
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "String")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Admin对象",value = "传入JSON数据",required = false) Admin admin,@PathVariable String id){
        //设置主键值
        admin.setId(id);
